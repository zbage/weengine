...
class DefaultModuleProcessor extends WeModuleProcessor {
    ...
    public function respond() {
        global $_W, $engine;
        $level = array();
        foreach ($_W['setting']['modules'][$_W['weid']] as $row) {
        ...
        if (!empty($_W['wechat']['default_period']) && empty($_W['cache']['default_period'])) {
        	return;
        }
        $response['FromUserName'] = $this->message['to'];
        $response['ToUserName'] = $this->message['from'];
        $response['MsgType'] = 'text';
        $default = pdo_fetchcolumn("SELECT `default` FROM " . tablename('wechats') . " WHERE `weid`=:weid", array(':weid' => $_W['wechat']['weid']));
