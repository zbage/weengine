...
class MusicModule extends WeModule {
	...
	public function fieldsFormSubmit($rid = 0) {
		global $_GPC, $_W;
		if (!empty($_GPC['title'])) {
			foreach ($_GPC['title'] as $index => $title) {
				if (empty($title)) {
					continue;
				}
				$update = array(
					...
					'url' => $_GPC['url'][$index],
					'hqurl' => $_GPC['hqurl'][$index],
				);
				if (!empty($_GPC['url-old'][$index]) && $_GPC['url'][$index] != $_GPC['url-old'][$index]) {
					file_delete($_GPC['url-old'][$index]);
				}
		...
		if (!empty($_GPC['title-new'])) {
			foreach ($_GPC['title-new'] as $index => $title) {
				if (empty($title)) {
					continue;
				}
				$insert = array(
