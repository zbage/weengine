...
class UserapiModuleProcessor extends WeModuleProcessor {
    ...
    public function respond() {
        global $_W;
    	$rid = $this->rule;
    	$module = $_W['setting']['modules']['userapi'];
    	...
    	if (!empty($module['settings']['sign'])) {
    		$sign = sha1($this->message['time'].$module['settings']['sign']);
    	}
    	...
    	$item['apiurl'] .= '&sign='.$sign;
    	$response = http_request($item['apiurl'], $GLOBALS["HTTP_RAW_POST_DATA"], array('CURLOPT_HTTPHEADER' => array('Content-Type: text/xml; charset=utf-8')));
    	$result = array();
