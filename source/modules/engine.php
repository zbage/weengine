...
class WeEngine {
	...
	public function __construct() {
		global $_W;
		if(empty($_W['wechat']['token'])) {
			exit('initial missing token');
		}
		$this->token = $_W['wechat']['token'];
	...
	private function matcher() {
		...
		} elseif (method_exists($this, 'matcher'.$this->message['type'])) {
			$response = call_user_func(array($this, 'matcher'.$this->message['type']));
		}
		return $response;
	}
	...
	public function before() {
		global $_W;
		runhook('before');
		if (!empty($_W['wechat']['default_period'])) {
			...
			if (!empty($row)) {
				if (TIMESTAMP - intval($row['lastupdate']) > intval($_W['wechat']['default_period'])) {
					$_W['cache']['default_period'] = TRUE;
				} else {
